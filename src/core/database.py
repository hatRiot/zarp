...
class Database(object):
	def __init__(self):
		...
		if db_type == 'sqlite3':
			self.connection = sqlite3.connect('config/zarp.db', check_same_thread = False)
		elif db_type == 'pgsql':
	...
	def initialize_schema(self):
		...
		insert('create table credentials (username, password, location, source_idx, time,\
								FOREIGN KEY(source_idx) REFERENCES host(ROWID));')
...
def fetch(query, parameters=None):
	...
	try:
		...
		if parameters is None: cursor.execute(query)
...
def insert(query, parameters=None):
	...
	try:
		...
		success = True
	except: success = False	# unique violation/doesnt exist/etc.
	return success
...
def dbcredentials(username,password,location,source):
	""" Insert credentials into the database.
		...
		Source should be an IP address of the source the credentials were coming from.
